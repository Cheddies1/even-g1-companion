...
- **Firmware-decomp investigation angles**: the G1 firmware decompilation added in commit 53929f8 (`docs/firmware-decomp-notes.md`, sourced from https://github.com/JohnRThomas/even_realities_decomp) gives three concrete leads that could turn detection from a guess into a real signal. These are the reason this is a spike-plus-fix rather than a one-line logging change.
  1. **We may be discarding a per-render status byte.** The decomp records that `deal_event_to_phone.c` emits `<opcode> C9 <state>` acks for at least `0x0D`, `0x0F` and `0x4E` - the text render opcode. Our code validates only the `C9`: `ret.data[1].toInt() != 0xc9` at `lib/ble_manager.dart:1088` and a similar check at `:1226`. `data[2]`, the `<state>` byte, is never read anywhere in `ble_manager.dart` (verified by grep). If that byte distinguishes "received" from "rendered" or carries a failure reason, we are throwing away exactly the signal that would have caught this. Establish what `<state>` means for `0x4E` before designing the fix.
  2. **Inter-leg SPI forwarding may make per-leg broadcast partly redundant.** `master_process_put_req.c` is documented as an inter-leg (SPI) forwarding subset, and there is a proven case: the `0x29` brightness readback is answered by forwarding the request over SPI to the other temple and returning its reply. The open question is whether the display opcodes (`0x4E` text, `0x52` streaming) are in that forwarded subset. If they are, a single-leg write may already reach both lenses and the symptom has a different cause entirely. If they are not, the analysis above stands unmodified. This question changes the shape of the fix, so settle it first.
  3. **Alternative silent-failure mode to rule out.** The decomp establishes that bytes 1-2 are a little-endian total length that the firmware validates, dropping the whole packet on mismatch with `packet length error, input data length = %d, packet data length = %d`. That is a per-packet drop rather than a per-lens one, so it does not obviously explain a one-eyed render, but it is the other known silent-drop path on this transport and worth eliminating.
- **Acceptance**:
  - [ ] Determine whether `0x4E` / `0x52` are in the `master_process_put_req.c` inter-leg forwarding subset, and record the answer in `docs/firmware-decomp-notes.md` and `docs/protocol-reference.md`.
  - [ ] Determine what the `<state>` byte in the `<opcode> C9 <state>` ack means for `0x4E`, and whether it can report a render failure.
  ...
  - [ ] If the `<state>` ack byte turns out to carry usable render status, the write path checks it rather than only `0xC9`.
  - [ ] Reproduce deliberately - force one leg unhealthy and confirm the new logging fires and says the right thing.
- **Notes**: Found while closing out the 2026-09-08 session, from a user report that looked like a one-off. Worth noting as a process point: the symptom was invisible in the logs, and only became findable because the app-level logs were clean enough to rule out every other explanation. Cross-ref `transport-honesty-smalls` and `ble-stability-tier3`. Do not bundle with either - this has a firmware-decomp dependency they do not.
